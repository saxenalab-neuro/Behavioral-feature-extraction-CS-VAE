...
def data_generator(idx, hdf5file,image_name,label_name,batch_size,if_train = True):
    i = 0
    while True:
        X = []
        Y=[]
        for b in range(batch_size):
            ...
            alll=idx[i]
            imn=image_name[alll[0]]
            lbn=label_name[alll[0]]
            try:
                ...
                if np.sum(x)==0 or np.sum(x) == np.inf or np.sum(x) == -np.inf or np.sum(y)==0 or np.sum(y)==np.inf or np.sum(y)==-np.inf:
                    x = (np.array(hdf5file[imn]['trial_0001'][0])/255).T  # incase of inf nan
                    x=np.rollaxis(x,1,0)
                    y = np.array(hdf5file[lbn]['trial_0001'][0])  # read dataset on the fly
            except:
                x = (np.array(hdf5file[imn]['trial_0001'][0])/255).T  # read dataset on the fly
                x=np.rollaxis(x,1,0)
                y = np.array(hdf5file[lbn]['trial_0001'][0])  # read dataset on the fly
